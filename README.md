...
│   └── seed.sql           # 20 cuisiniers fictifs, 50 plats, 40 avis, 4 commandes de démo
├── scripts/
...
- **Carte** (Leaflet/OSM) centrée sur Paris avec 20 cuisiniers répartis sur 8 arrondissements (9e, 10e, 11e, 13e, 18e, 19e, 20e, 3e)
- **Recherche & filtres** : par texte libre, type de cuisine, prix max, tri par note/distance/prix (distance calculée via géolocalisation navigateur si autorisée, sinon centre de Paris par défaut)
- **Fiche cuisinier** : photo, note, spécialité, plats du jour, avis
- **Fiche plat** : détail, sélecteur de quantité, bouton **Commander** → flux de commande simulé (mention claire "démo — paiement simulé")
